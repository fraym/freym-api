...
export interface DeliveryClient {
    ...
    getViewData: <T extends object>(
        view: string,
        authData: AuthData,
        filter?: Filter
    ) => Promise<T | null>;
    getDataList: <T extends object>(
        projection: string,
        authData: AuthData,
        limit?: number,
        page?: number,
    ...
    getViewDataList: <T extends object>(
        view: string,
        authData: AuthData,
        limit?: number,
        page?: number,
...
export const newDeliveryClient = async (config?: DeliveryClientConfig): Promise<DeliveryClient> => {
    ...
    const getViewData = async <T extends object>(
        view: string,
        auth: AuthData,
        filter: Filter = { fields: {}, and: [], or: [] }
    ): Promise<T | null> => {
    ...
    const getDataList = async <T extends object>(
        projection: string,
        auth: AuthData,
        limit: number = 0,
        page: number = 1,
    ...
    const getViewDataList = async <T extends object>(
        view: string,
        auth: AuthData,
        limit: number = 0,
        page: number = 1,
    ...
    const upsertData = async <T extends object>(
        projection: string,
        authData: AuthData,
        dataId: string,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
